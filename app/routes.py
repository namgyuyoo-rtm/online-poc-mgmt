...
async def send_email(project, email_type, additional_data=None):
    ...
    try:
        ...
        # 프로젝트 상태 검증
        if config.get('status') and email_type == EmailType.SUBMISSION_COMPLETE:
            # SUBMISSION_COMPLETE는 APPLICATION_COMPLETE 상태에서도 발송 가능
            if project.status != ProjectStatus.APPLICATION_COMPLETE and project.status != ProjectStatus.RECEPTION_COMPLETE:
                logger.error(f"SUBMISSION_COMPLETE 이메일은 APPLICATION_COMPLETE 또는 RECEPTION_COMPLETE 상태에서만 발송 가능합니다. 현재 상태: {project.status.value}")
                return False, f"현재 상태({project.status.value})에서는 이메일을 발송할 수 없습니다"
        elif config.get('status') and project.status != config['status']:
...
async def send_email_api(project_id, email_type):
    ...
    try:
        ...
        try:
            ...
            # 특별 처리: SUBMISSION_COMPLETE 이메일 전송 시 상태 업데이트
            if email_type_obj == EmailType.SUBMISSION_COMPLETE and request_data.get('update_status'):
                if project.status == ProjectStatus.APPLICATION_COMPLETE:
                    ...
                    status_history = StatusHistory(
                        ...
                        created_by='System',
                        reason='접수 완료 이메일 발송에 의한 자동 상태 변경'
                    )
                    db.session.add(status_history)
                    db.session.commit()
...
def validate_project_status(project_id):
    ...
    try:
        ...
        if expected_status and project.status.value != expected_status:
            # SUBMISSION_COMPLETE 이메일을 위한 특별 검증
